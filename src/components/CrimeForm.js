...
import useCrimeForm from "../hooks/useCrimeForm";
...
const CrimeForm = () => {
  ...
  const [LeftFaceError, setLeftFaceError] = useState("");
  const [rightFaceError, setRightFaceError] = useState("");
  const { pending, error, response, crimeFormFirebase } = useCrimeForm();
  ...
  const handleLeftFace = (e) => {
    const selectedFile = e.target.files[0];
    if (!selectedFile) {
      setLeftFaceError("Please select a front face image.");
      return;
    }
    if (!selectedFile.type.includes("image")) {
      setLeftFaceError("Please select a image file.");
      return;
    }
    if (selectedFile.size > 100000) {
      setLeftFaceError("Image size should be less than 100kb.");
      return;
    }
    setLeftFaceError(null);
    setLeftFace(selectedFile);
  };
  const handleRightFace = (e) => {
    const selectedFile = e.target.files[0];
    if (!selectedFile) {
      setRightFaceError("Please select a front face image.");
      return;
    }
    if (!selectedFile.type.includes("image")) {
      setRightFaceError("Please select a image file.");
      return;
    }
    if (selectedFile.size > 100000) {
      setRightFaceError("Image size should be less than 100kb.");
      return;
    }
    setRightFaceError(null);
    setRightFace(selectedFile);
  };

  const handleCrimeForm = (e) => {
    e.preventDefault();
    if (
      frontFace.name === leftFace.name ||
      frontFace.name === rightFace.name ||
      leftFace.name === rightFace.name
    ) {
      setFrontFaceError("Image names must be unqiue.");
      return;
    }
    crimeFormFirebase(
      name,
  ...
  return (
    <div className=" bg-gray-900 text-gray-50 py-12 sm:py-16 md:py-24 lg:pt-24 lg:pb-36">
      <div className="text-left mx-auto max-w-screen-xl pb-12 px-4 items-start gap-16 lg:flex md:px-8">
        <form className="lg:w-2/4" onSubmit={handleCrimeForm}>
          ...
          {/* File Error Message */}
          {(frontFaceError || LeftFaceError || rightFaceError) && (
        ...
        <div className="hidden lg:block w-2/4 pt-8">
          <img src={crime} alt="crime" className="w-full" />
        </div>
      </div>
    </div>
  );
